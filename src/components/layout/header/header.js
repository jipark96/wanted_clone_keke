...
function Header() {
  ...
  let [headerLi,setHeaderLi] = useState(["채용","이벤트","직군별 연봉","이력서","프리랜서","AI 합격예측"]);
      ...
      <div className="header_wrap">
        <div className="header_content">
          <img
            ...
            alt=""
          ></img>
          <div className="header_p1" onClick={() => {
            navigate('/')
          }}>wanted</div>
        </div>
        <span className="header_ul">
          {
            headerLi.map((a,i) => {
              return (
              <div className="header_li">{headerLi[i]}</div>
              )
            })
          }
        </span>
        <span className="header_span">
          ...
          {loginModal && (
            <LoginModal loginModal={loginModal} setLoginModal={setLoginModal} joinModal={joinModal} setJoinModal={setJoinModal}/>
          )}
          {
              joinModal && <JoinModal joinModal={joinModal} setJoinModal={setJoinModal}/> 
            }
